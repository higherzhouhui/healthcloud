<template>
	<view class="container">
		...
		<view class="content">
			...
			<view class="bg-video">
				<image @tap="vplay" src="../../static/home/cover.png" v-show="!played || ruleVisible || isShowProgress"
					class="video"></image>
				<video id="myVideo" v-show="played && !ruleVisible && !isShowProgress" class="video" ref="video"
					:src="videoUrl" loop controls :show-mute-bt="true" play-btn-position="middle"
			...
			getHomeData() {
				Promise.all([getBannerListRequest(), getNewsListRequest({
					...
					if (baseInfo.code === 200) {
						const datas = baseInfo.data
						this.videoUrl = datas.video
						this.gfql = {
							equityCert: datas.equityCert,
							blogs: datas.blogs,
							microphygmia: datas.microphygmia,
							microphygmiaDown: datas.microphygmiaDown
						}
						console.log(this.gfql)
						Local('videoUrl', datas.video)
						Local('gfql', this.gfql)
						Local('baseInfo', datas)
					}
